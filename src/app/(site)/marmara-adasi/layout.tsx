...
const TITLES: Record<string, string> = {
  '/marmara-adasi/tarihce': 'Tarihçe',
  '/marmara-adasi/gunumuz': 'Günümüz',
};

const BANNER_TITLES: Record<string, string> = {
  '/marmara-adasi/tarihce': 'Marmara Adası Tarihçesi',
  '/marmara-adasi/gunumuz': 'Günümüzde Marmara Adası',
};

export default function MarmaraAdasiLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const title = TITLES[pathname as string] || 'Marmara Adası';
  const bannerTitle = BANNER_TITLES[pathname as string] || title;
  
  const parentCategory = "Marmara Adası";
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-24">
      ...
      >
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-full relative z-10 flex flex-col">
          ...
          <div className="pt-8 md:pt-10 flex flex-wrap items-center gap-2 text-white/80 text-xs sm:text-sm font-medium w-full">
            ...
            <span className="text-white/80 whitespace-nowrap">{parentCategory}</span>
            <ChevronRight size={14} className="flex-shrink-0" />
          ...
          <div className="flex-1 flex flex-col items-center justify-center pb-20 md:pb-28 w-full px-2 sm:px-4">
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tight text-center drop-shadow-md leading-tight text-balance max-w-4xl mx-auto">
              {bannerTitle}
            </h1>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto -mt-20 md:-mt-28 px-4 sm:px-6 relative z-20">
        <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] border border-slate-200 min-h-[500px] relative">
